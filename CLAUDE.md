...
A single-page, scroll-driven Islamic wedding invitation ("Shuhaib & Farha") built on TanStack Start (SSR) and deployed to Netlify. There is exactly one user-facing route: `/`.

The site is a digital rendering of a printed invitation card. **The card names the groom first, so the site does too — hero, details panel, finale and the calendar link all pair groom then bride.** The config still keys them `groom` / `bride` semantically; only the render order is fixed. (An earlier invitation on this codebase was bride-first because the bride's family hosted — if you swap couples again, check which way the printed card reads rather than assuming either.)
...
- `groom.shortName` / `bride.shortName` feed both envelopes' letter, the finale and the calendar link; `displayName` (upper case) feeds the hero card; `monogram` feeds the curtain seal.
- `ceremonyName` is the card's own wording for the event ("Wedding Ceremony"), and it is the details panel's `SectionTitle`, the hero's `{ceremonyName} · {time}` line and the calendar entry's summary. Don't append "Details" to it in the title — that reads as a tautology.
- `venue.name` + `venue.location` are what render; `venue.address` is **not** rendered and exists only to populate the calendar entry's location field, so it may carry district/state that the printed card omits.
- **This card supplies no parents, grandparents, residences, host names or host phone**, so those fields are gone from the config and the Families panel and finale signature block were removed with them. If a future card carries that data, restore the fields _and_ the panel together — the `.person-card*` / `.person-row*` / `.family-link` / `.rv-l` / `.rv-r` CSS is still in [styles.css](src/styles.css) and unused, waiting for exactly that.
...
[index.tsx](src/routes/index.tsx) wraps the six sections in [StackScroll](src/components/shared/StackScroll.tsx). Every section carries `.panel`; each one overlaps the previous by `-2rem` with a rounded top edge and an upward shadow, and a scrubbed ScrollTrigger scales + dims the outgoing panel as the next covers it. Scrolling up reverses it exactly.
...
**That windowing is the whole point, so don't "simplify" it to `position: sticky` or ScrollTrigger's `pin`.** Either of those pins a panel for its full height, which caps it at one viewport of _visible_ content — and the details panel and the finale both run two to three screens tall on mobile, so everything below their first screen would become unreachable. Here they scroll normally right up until the handoff. Transforms don't affect layout, so displacing a panel a full screen adds no scroll height.
...
Current rhythm is kraft ↔ black card stock: cover (dark), welcome (`pat-light`), details (`pat-mid`), countdown (dark), venue (`pat-mid`), finale (dark).

**Six panels can't alternate strictly** — the deck must open _and_ close on black, which needs an odd count. Details is therefore `pat-mid` rather than `pat-light`: welcome → details is the one kraft-on-kraft boundary, and stepping the tone is what keeps the seam legible alongside the rounded top edge and upward shadow. Add or remove a panel and re-check this whole sequence; two adjacent panels sharing a tone read as one continuous surface and the handoff stops being visible.

### Page composition

[src/routes/index.tsx](src/routes/index.tsx) is the whole app: `MusicProvider` → `ScrollProgress` → `SmoothScroll` → six sections → `MusicWidget`. Section order there is the scroll order.

Each section is a self-contained `<section id="…">` that:

1. Reads its copy from `wedding`.
2. Registers its own GSAP reveal in `useGSAP(..., { scope: rootRef })`, animating the shared `.rv` class via a `ScrollTrigger` on `rootRef`. Use the `REVEAL` preset from [src/lib/gsap.ts](src/lib/gsap.ts) (`REVEAL.from` / `REVEAL.to` / `.stagger` / `.start`) so every band moves with one voice. `.rv-l` / `.rv-r` are the directional variants, currently unused (they belonged to the removed family cards) but still pre-hidden by [gsap.ts](src/lib/gsap.ts).
...
| Content  | `.details-*`, `.detail-cell*`, `.count-cell*`, `.venue-map` (`.person-card*` and `.signature` are retained but currently unrendered)                                                                                                      |
