...
"View Invitation" on the hero card calls `autoScrollToEnd()` in [SmoothScroll.tsx](src/components/shared/SmoothScroll.tsx), which plays the whole card through to the foot of the page **one screenful at a time, pausing on each**. The scroll cue below it still just steps to `#details`.

- **The pause is the feature; the travel between pauses is just transport.** Each stop is reached over `STEP_SECONDS` (1.6) and held for `DWELL_SECONDS` (2). This replaced a single continuous glide at a constant 270px/s, and it is not merely a slower version of it: a continuous pan gives every line the same fraction of a second in the comfortable middle of the screen, where dwelling holds a whole screenful still long enough to read.
- **Every panel top is a stop, and `buildStops` resolves the whole list up front.** A panel top is the one position where the panel is fully composed and its handoff has not begun, so it is the only place a pause shows a clean page instead of one already sliding under the next. Stepping by a fixed fraction of the viewport — which this did first — lands wherever it happens to land, so pauses fell mid-sentence and mid-handoff. Resolving the list once also avoids the drift that accumulates when each step is measured from the live scroll position.
- **Panels taller than a screen get filler stops**, or the 2–3 screen details panel would be one uninterrupted slide. Fillers travel `STEP_VIEWPORT_RATIO` (0.85) of a viewport — shy of a full screen so the last line or two stays visible — and one is skipped if it would land within a quarter screen of the next panel top, which reads as a stutter right before the real stop.
- Measured on a phone-height page: pauses at 844, 1560, 2279, 3167, 4011, 4333, each held 1993–2102ms, ending on the limit — 844/3167/4011 being the three panel tops.
- **`stepEase` is a sine ease-in-out, and that reverses the old rule.** The continuous tour deliberately avoided an ease-in-out because over a very long travel its peak velocity is twice the average and the middle whipped past. A step is short and _should_ start and stop; sine keeps the peak to ~1.57x the average, gentler at both ends than a cubic.
- **Two different mechanisms cancel the tour, one per state — don't collapse them.** _Mid-step_, a wheel or touch is handled by Lenis itself: it routes them through `scrollTo(targetScroll + delta)`, which replaces the running animation, so our `onComplete` never fires, no next step is queued, and the tour stops. (It also pins `targetScroll` to the current animated position during a _programmatic_ scroll — `onUpdate` in lenis.mjs — so the reader resumes from where the tour reached rather than being flung at its destination.) _Mid-dwell_, nothing is animating and there is no animation to replace, so the pending timer must be cancelled explicitly; that is what the wheel/touch/key listeners are for. Without them a nudge during a pause is overridden a moment later by the next step. Verified by dispatching a real wheel event 2.6s in, mid-dwell: the tour never advanced past the first step.
...
**No panel may be shorter than the screen, and `.panel`'s `min-height` is what guarantees it.** A panel's handoff runs from `docTop(next) - viewport` to `docTop(next)`. If the panel is shorter than the viewport that start lands _above its own top_, so by the time the panel arrives the next one is already covering it — the countdown was 441px against an 844px viewport and was half-buried under the finale the entire time it was on screen. The minimum is `calc(100svh + var(--panel-overlap))`, not `100svh`: each panel is pulled up over the last by `--panel-overlap`, so at a flat screen height the handoff still began 32px early. Both the negative margin and the min-height read that one variable — they have to agree exactly, hence the variable rather than two `2rem`s. `.env-scene` keeps its own `100svh` (later in the file, so it wins) because the cover→details boundary is flush.
