...
- **One tween, one ScrollTrigger per panel**, with `scrub: 0.6`. The small scrub value low-pass-filters the scroll so the transform doesn't vibrate against the scroller's sub-pixel position (the flicker reported as the next page nears the top). `scrub: true` (pixel-exact) is what jittered — the transform is written from a main-thread scroll event while the page itself scrolls on the compositor, so exactness just faithfully reproduces the desync.
