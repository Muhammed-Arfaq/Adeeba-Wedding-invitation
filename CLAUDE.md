...
Current rhythm is navy ↔ petrol, across four panels: cover (`pat-dark`), details (`pat-petrol`), countdown (`pat-dark`), finale (`pat-petrol`). **There is no light band.**

Going all-dark dissolved a parity problem the old light/dark flip had: four panels that must open _and_ close dark cannot alternate strictly, so one boundary always repeated a tone. Two dark hues alternate cleanly, and the seam is carried by the hue step plus each panel's rounded top edge and upward shadow. Two adjacent panels sharing a band read as one continuous surface and the handoff stops being visible, so re-check the sequence whenever a panel is added or removed.

Because every band is dark there is now **one** foreground set, declared on `.panel` and `.panel--dark` together. Keep `.panel--dark` on every panel even though it is no longer a variant: a pile of rules select through it (`.panel--dark .card`, `.panel--dark .paper-grain::after`, `.panel--dark .shimmer`), and it is the hook to restore a light band with.
...
| Surfaces | `.pat-dark`, `.pat-petrol`, `.paper-grain`, `.card`                                                                                                                                                                                       |
...
Three families: **navy**, **petrol**, **gold**. Nothing else — resist adding a fourth hue. The page is dark throughout; the only light on it is gold.

Navy `--color-navy-900` `#0b1622` → `--color-navy-600` `#253c55`. Petrol `--color-petrol-900` `#061a20` → `--color-petrol-700` `#12363f` — hue 196 against navy's 212, close enough to read as the same family, far enough that the seam between two panels is visible. Gold `#c9a44c`, light `#e8cf94`, pale `#f5e6bf`, deep `#785a1d`.

Text is `--fg` `#f2e8d2`, a gold-lit cream rather than white, so **nothing on the page is a neutral**; `--accent` is `#e0c07a` and every rule, hairline, border and label is gold. Reach for the `.t-*` utilities rather than the hexes.

**The beige ramp still exists and is still used — but only by the envelope**, which is a photographed physical object rather than UI: kraft body, navy flap, gold wax. The card _inside_ it (`.env-letter`, `.env-photo__pocket`) is dark, matching the `.hero-card` it becomes, so opening a kraft envelope reveals a dark gold-edged card. Don't "finish the job" by recolouring the envelope body unless asked — a beige exterior is what makes the reveal read.

**Glass, and why it has no `backdrop-filter`.** Every raised surface shares one recipe, declared as `--glass` / `--glass-border` / `--glass-hi` / `--glass-shadow` on `.panel`: a translucent white gradient, a gold hairline, an inset top highlight and a deep drop shadow, plus a `::before` sheen that falls steeply from the top edge. That sheen is what sells it — an edge-lit pane reads as glass, a flat translucent rectangle reads as a tint. A real blur is ruled out by the scrub-cost rule in **The stack**, and would in any case return nearly the pixels it was given, since these backdrops are smooth gradients. `.detail-cell` deliberately uses a _lighter_ mix than `--glass`: it sits inside `.details-panel`, and stacking the full recipe twice muddied both sheets.

**Check contrast by computing it, not by eye** — composite alpha foregrounds over the actual band first, and remember the glass itself lightens the background it sits on. All twenty pairs (both bands × band/glass × `--fg`, `--fg-2`, `--fg-3`, `--accent`, the countdown numerals) clear AA, the tightest being `--fg-3` on glass over petrol at 4.71:1.
...
#### Recolouring the plates (kraft/black → beige/navy)

**The shipped plates are recoloured, and the originals in git history are still kraft and black.** Re-derive from those, not from the shipped files — recolouring twice compounds. The recolour is another throwaway canvas script; four things about it were learned the hard way and will bite anyone repeating it:
