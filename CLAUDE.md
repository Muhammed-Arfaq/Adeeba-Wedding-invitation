...
Smoothness is fragile here and was hard-won — these keep it fluid, and undoing any one brings back lag or flicker:
- **One tween, one ScrollTrigger per panel**, with `scrub: 0.6` and `force3D: true`. The small scrub value low-pass-filters the scroll so the transform doesn't vibrate against Lenis's sub-pixel scroll position (the flicker reported as the next page nears the top); `force3D` keeps the animating panel on a single GPU layer rendered sub-pixel. `scrub: true` (pixel-exact) is what jittered.
...
- **No permanent `will-change` on `.panel`.** Promoting all seven full-screen panels to GPU layers at once strains mobile GPUs and can itself flicker; `force3D` promotes only the one panel currently animating.
...
- **The envelope (`.env-photo` / `.env-stage`) is pinned to the viewport centre, not the grid** — `position: absolute; left: 50%; top: calc(var(--app-height, 100svh) / 2); translate: -50% -50%`. It's centred on **`--app-height`, which EnvelopeCover sets to `window.innerHeight`**, not on `svh`: mobile browsers' `svh`/`lvh`/`dvh` disagree with the real visible height while the URL bar is showing, which left the envelope visibly off-centre on real phones even though every emulator test passed. `window.innerHeight` is the reliable value. `100svh` is only the SSR/no-JS fallback. The scene keeps `min-height: 100svh` (not `--app-height`) so its background doesn't reflow when the URL bar toggles after opening. `top` is measured from the scene's top edge, which is the document top while sealed (scroll locked), so this is exactly the viewport centre and, critically, **independent of the scene's height**. Only the card (`.env-slot`) is a grid item, so the scene still grows to fit it. This decoupling fixes two reports at once: the envelope centres on every viewport regardless of the ~683px card, and it no longer *moves down while opening* (previously the card returning to flow inflated the scene and re-centred the grid-placed envelope mid-animation). `translate` is a separate property from `transform`, so the GSAP drift/open tweens compose with the centring instead of wiping it. The cover→first-section boundary is also flush (`.env-scene + .panel { margin-top: 0 }`) so the cover's stack tween doesn't pre-start at scroll 0.
